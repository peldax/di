...
test(function () { // nullable enum
	$expectation = Expect::enum(Expect::string(), true)->nullable();
	...
	Assert::exception(function () use ($expectation) {
		$expectation->complete(false);
	}, InvalidConfigurationException::class, 'The option expects to be string|true|null, false given.');
});
